...
func (b *Builder) View() *tview.TreeView {
	...
	buildImage := tview.NewTreeNode(tview.TranslateANSI(" build:" + boldWhite.Sprint(" paketo-buildpacks/build:full-cnb") + bold.Sprintf(" - d0e8bfb686fd") )).SetColor(tcell.ColorDimGray)
	runImage := tview.NewTreeNode(tview.TranslateANSI(" run:" + boldWhite.Sprint(" paketo-buildpacks/run:full-cnb") + bold.Sprintf(" - 84eff6936799")  + boldGreen.Sprintf(" (update available)") )).SetColor(tcell.ColorDimGray)
